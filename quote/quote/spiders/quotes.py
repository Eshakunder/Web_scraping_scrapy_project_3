...
import scrapy
from ..items import QuoteItem
from scrapy.utils.response import open_in_browser #to open the response in the browser to check if we are able to login to the website or not. if we are able to login to the website then we will see the quotes on the page. if we are not able to login to the website then we will see the login page again. so this is a good way to check if we are able to login to the website or not.
from scrapy.http import FormRequest
class QuotesSpider(scrapy.Spider):
    name = "quotes"

    allowed_domains = ["toscrape.com"]
    start_urls = ["https://quotes.toscrape.com/login"] #list of url to scrape.


    def parse(self, response):
        open_in_browser(response) #to open the response in the browser to check if we are able to login to the website or not. if we are able to login to the website then we will see the quotes on the page. if we are not able to login to the website then we will see the login page again. so this is a good way to check if we are able to login to the website or not.
        token = response.css('form input::attr(value)').extract() #extract the token value from the form input
        #print(token) # to print csrf token value in the terminal
        return FormRequest.from_response(response, formdata={
            'csrf_token':token,
            'username':'abc@gmail.com',#emailand password should be correct one which u use to login to the website. otherwise it will not work. if you use wrong email and password then it will not login to the website and it will not scrape the data from the website. so make sure to use correct email and password to login to the website.
            'password':'123123'
            },callback=self.start_scraping) #to send a post request to the server with the login credentials and the token value
        #to send a post request to the server with the login credentials and the token value
